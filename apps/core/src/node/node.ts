...
import { MessageEvent, Packet, SendableMessage } from 'src/types/types';
...
const Server = () => {
    ...
    */
   const _send = (connectionId: string, message: SendableMessage) => {
   ...
   const closeAll = (callback?: () => void) => {
      for (let [_connectionId, socket] of connections) {
         socket.destroy();
      }
   ...
   const findNodeId = (connectionId: string) => {
      for (let [nodeId, neighborConnectionId] of peers) {
   ...
   emitter.on('_connect', (connectionId) => {
      _send(connectionId, { type: 'handshake', data: { NODE_ID } });
   });
   ...
   const sendPacket = (packet: Packet) => {
      ...
      for (const nodeId of peers.keys()) {
         send(nodeId, packet);
      }
   };
    ...
    */
   const broadcast = (message: SendableMessage, id = nanoid(), origin = NODE_ID, ttl = 255) => {
